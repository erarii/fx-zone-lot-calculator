...
def fetch_usdjpy():
    try:
        ...
        return float(usd_jpy) if usd_jpy else None
...
def calc_positions(pair, direction, division, weights, avg_price, max_loss, stop, upper, lower, usd_jpy_rate=1.0):
    ...
    # 損失計算（USD建てペアは後でJPY換算）
    if direction=="buy":
        loss_per_unit = [max(0, p - stop) for p in prices]
    else:
        loss_per_unit = [max(0, stop - p) for p in prices]

    total_loss_usd = sum([w*unit*l for w,l in zip(weights, loss_per_unit)])

    # USD建て通貨ならJPY換算
    if not pair.endswith("JPY") and pair != "GOLD":
        total_loss = total_loss_usd * usd_jpy_rate
    else:
...
if st.button("計算"):
    ...
    usd_jpy_rate = None
    if not pair.endswith("JPY") and pair != "GOLD":
        with st.spinner("USD/JPY を自動取得中…"):
            usd_jpy_rate = fetch_usdjpy()
        if usd_jpy_rate is None:
            st.warning("USD/JPY レート取得失敗。手動入力が必要です。")
