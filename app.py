...
LOT_INFO = {"FX": 10000,"GOLD": 1}  # FX:1万通貨、GOLD:1oz
...
def get_pair_rate(pair, rates, usd_jpy):
    if pair=="GOLD":
        ...
        if base=="USD":
            ...
            # XXXJPY = USDJPY / (1USD=XXX)
            return usd_jpy / float(rates[base])
        else:
            return usd_jpy
    else:  # ドル建てFX
        base, quote = pair[:3], pair[3:]
        if quote=="USD" and base in rates:  # XXX/USD
            return float(rates[base])
        elif base=="USD" and quote in rates:  # USD/XXX
            return float(rates[quote])
        elif base in rates and quote in rates:  # XXX/YYY
...
def calc_positions(pair,direction,division,weights,avg_price,max_loss,stop,upper,lower,usd_jpy_rate):
    ...
    loss_per_unit=[]
    for p in prices:
        diff = stop-p if direction=="buy" else p-stop
        diff=abs(diff)
        # 円換算
...
if st.button("計算"):
    ...
    try:
        result=calc_positions(pair,direction,division,[float(w) for w in weights],avg_price,max_loss,stop,upper,lower,usd_jpy_rate)
        st.subheader("計算結果")
